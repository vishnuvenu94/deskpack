...
export function createBackendPortShim(): string {
  return `// Generated by deskpack - redirects hardcoded backend listen ports to the selected desktop port.
{
  const preferredPort = Number(process.env.DESKPACK_PREFERRED_BACKEND_PORT || 0);
  const actualPort = Number(process.env.DESKPACK_BACKEND_PORT || process.env.PORT || 0);
  if (
    preferredPort > 0 &&
    actualPort > 0 &&
    ...
    function maybeLogRewrite() {
      if (loggedRewrite) return;
      loggedRewrite = true;
      console.warn(
        "[deskpack] Redirecting backend listen port " +
          preferredPort +
          " to " +
          actualPort +
          " because the preferred port was unavailable.",
      );
    }

    function patchListen(serverCtor) {
      if (!serverCtor || !serverCtor.prototype) return;
      const originalListen = serverCtor.prototype.listen;
      if (typeof originalListen !== "function") return;
      if (originalListen.__deskpackPortShim) return;

      function deskpackListen(...args) {
        if (args.length > 0) {
          if (normalizePort(args[0]) === preferredPort) {
            args[0] = actualPort;
            maybeLogRewrite();
          } else if (
            args[0] &&
            ...
            normalizePort(args[0].port) === preferredPort
          ) {
            args[0] = { ...args[0], port: actualPort };
            maybeLogRewrite();
          }
        }
      ...
      deskpackListen.__deskpackPortShim = true;
      serverCtor.prototype.listen = deskpackListen;
    }

    patchListen(require("node:net").Server);
    patchListen(require("node:http").Server);
    patchListen(require("node:https").Server);
  }
}`;
}
